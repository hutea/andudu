...
public abstract class DAOSupport<T> implements DAO<T> {
	...
	public long getCount() {
		return (Long) em.createQuery(
				"select count(" + this.getCountField(this.entityClazz) + ") from "
						+ buildEntityName() + " o").getSingleResult();
	}
	...
	public QueryResult<T> getScrollData(int startIndex, int maxresult, String jpql,
		...
		query = em.createQuery("select count(" + this.getCountField(this.entityClazz) + ") from "
				+ entityName + " o " + whereJpql);
		if (jpql != null && !"".equals(jpql)) {
			this.setParms(query, params);
		}
			...
			for (PropertyDescriptor propertydesc : propertyDescriptors) {
				...
				if (method != null && method.isAnnotationPresent(EmbeddedId.class)) {
					PropertyDescriptor[] ps = Introspector.getBeanInfo(
							propertydesc.getPropertyType()).getPropertyDescriptors();
					out = "o."
